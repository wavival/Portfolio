...
- **Bilingual routing.** ES is the default at root with Spanish slugs (`/proyectos`, `/servicios`, `/sobre-mi`, `/contacto`, `/herramientas`, `/privacidad`); EN mirrors under `/en/` with English slugs. Never hardcode a route that ignores language: use the `isEn ? "/en/..." : "/<es-slug>"` ternary in components, and update the slug map in `src/i18n/utils.ts` (`EN_PAGE_MAP` + `getAltLangUrl`) whenever you add a page or rename a slug. Legacy English-word ES routes 301-redirect to the Spanish slugs in `netlify.toml`.
